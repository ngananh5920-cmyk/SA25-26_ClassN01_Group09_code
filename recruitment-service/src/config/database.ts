...
const connectDB = async (): Promise<void> => {
  try {
    ...
    console.log('Connecting to MongoDB (recruitment-service)...');
    ...
    console.log(`MongoDB connected (recruitment-service): ${conn.connection.host}`);
    console.log(`Database: ${conn.connection.name}`);
  } catch (error: any) {
    console.error('Database connection error (recruitment-service):');
