...
const connectDB = async (): Promise<void> => {
  try {
    ...
    console.log('Connecting to MongoDB (attendance-service)...');
    ...
    console.log(`MongoDB connected (attendance-service): ${conn.connection.host}`);
    console.log(`Database: ${conn.connection.name}`);
  } catch (error: any) {
    console.error('Database connection error (attendance-service):');
